...
export async function convertToAzureChatMessages(
    ...
    if (message.role === "user") {
      ...
      messages.push({
        ...
        content: await Promise.all(
          message.content.map(async (part, index) => {
            switch (part.type) {
              case "text": {
                return {
                  type: "text",
              ...
              case "image": {
                ...
                if (imagePart.image instanceof URL) {
                  const imgUrl = imagePart.image.toString();
                  const res = await fetch(imgUrl);

                  if (!res.ok) {
                    throw new Error(
                      `Failed to fetch ${imgUrl}: ${res.status} ${res.statusText}`
                    );
                  }
